"""
perturbation.py — Mid-episode kinematic leg-lock for the gecko rollout.
...
Usage (in run_es_dynamic.py — see INTEGRATION block at the bottom):
    ...
    leg_lock = LegLock(
        ...
        patterns=[".*LF.*"],     # <- fill from inspect_robot.py joint names
    )
...
class LegLock:
    ...
    patterns:
        ...
        match all joints of one leg, e.g. ``".*LF.*"``. Provide several to
        lock multiple legs at once (e.g. the paper's LF+RF condition:
        ``[".*LF.*", ".*RF.*"]``). Resolve these against the real joint names
        printed by ``inspect_robot.py`` — do not guess.
    hold_position:
        Where to pin the locked joints. ``"default"`` uses
        ``robot.data.default_joint_pos`` (the constant-default-position
        semantics of Leung et al.). A float tensor of shape
