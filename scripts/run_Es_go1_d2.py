"""
run_es_go1_d2.py -- Objective 2: META-LEARNED temporal window.

Sibling of run_es_go1.py. Identical env, reward, ES optimiser, curriculum,
action filter, seeding, config capture and checkpointing. The ONLY substantive
differences are:
...
SHARING (Objective 3 is a one-line change to SHARE below)
    ...
    'layer'   1 theta per activation node         <- Objective 3
    'node'    1 theta per unit per node           <- Objective 3
"""
...
def main():
    ...
    print(f"\n[INFO] Initializing D2 Network (obs={obs_dim}, act={action_dim})...")
    # The single structural change from run_es_go1.py, which reads
    ...
    for epoch in range(EPOCHS):
        ...
        # --- D2-SPECIFIC: the evolved temporal window ------------------------
        # The entire addition over run_es_go1.py. The trajectory of M_eff over
        # generations is the headline result of Objective 2, so it is logged
        # every epoch. Two distinct things are recorded and they must not be
        # conflated in the write-up:
        #   D2/pop_* : statistics over the SAMPLED population, i.e. mu + sigma*eps.
        #              Useful for spread, but it carries the exploration noise.
        #   D2/mu_*  : the SEARCH DISTRIBUTION MEAN, read off the theta tail of
        #              solver.mu. This is what is carried to the next generation
